...
export const renderRegisterPage = (req, res) => {
  // Let the frontend know if a registration secret is required
  const secretRequired = !!config.settings.registrationSecret;
  res.render("register", { secretRequired });
};
...
export const registerUser = async (req, res) => {
  try {
    ...
    req.session.regenerate((err) => {
      ...
      return res.status(201).json({ redirectUrl: "/upload" });
    });
...
export const logoutUser = (req, res) => {
  ...
  req.session.destroy((err) => {
    if (err) {
      ...
      return res.status(500).render("error", { errorMessage: "Could not log out." });
    }
